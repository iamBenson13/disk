            ...
            if (isShift(e) && document.activeElement.id != "sreachKeyWordIn") {
                switch (keyCode) {
                    ...
                    case 85:
                        ...
                        break;
                    case 80:
                        $('#createFeatureButtonLi a').click();
                        break;
                    case 81:
                        $('#renameFeatureButtonLi a').click();
                        break;
                    case 68:
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(countryView, function (i, item) {
                ...
                $("#areaX").append("<option value=" + item.countryId + ">" + item.countryName + "</option>");
            });
        },
        error: function () {
        }
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            $.each(featureView, function (i, item) {
                ...
                $("#ffX").append("<option value=" + item.featureId + ">" + item.featureName + "</option>");
            });
            originFeatureView = $.extend(true, {}, featureView);
        },
        error: function () {
        }
    });
...
function showAccountView(folderView) {
    ...
    if (authList != null) {
        ...
        if (checkAuth(authList, "C")) {
            $("#createFeatureButtonLi").removeClass("disabled");
            $("#createFeatureButtonLi a").attr("onclick", "showNewFeatureModel()");
        }
...
function showFolderTable(folderView) {
    ...
    $
        .each(
            featureView,
            function (n, fi) {
                fi.featureName = fi.featureName.replace('\'', '&#39;');
                var fileRow = "<tr id=" + fi.featureId + " onclick='checkfile(event," + '"'
                    + fi.featureId + '"' + ")' ondblclick='checkConsFile(event," + '"' + fi.featureId + '"' + ")' id='" + fi.featureId
                    + "' class='filerow'><td>" + fi.featureName
                    + "</td>";
                var list = fi;
                var names = "";
                ...
                fileRow = fileRow + names;
                // fileRow = fileRow + fi.featureList;

                fileRow = fileRow + "</td>" + "<td>";
                if (aD) {
                    fileRow = fileRow
                        + "<button onclick='showDeleteFeatureModel("
                        + '"'
                        + fi.featureId
                        + '","'
                        + fi.featureName
                ...
                if (aR) {
                    fileRow = fileRow
                        + "<button onclick='showRenameFeatureModel("
                        + '"'
                        + fi.featureId
                        + '","'
                        + fi.featureName
...
//显示删除功能模态框(YBS)
function showDeleteFeatureModel(featureId, featureName) {
    $('#deleteFeatureBox')
        .html(
            "<button id='ddbutton' type='button' class='btn btn-danger' onclick='deleteFeature("
            + '"' + featureId + '"' + ")'>删除</button>");
    $("#ddbutton").attr('disabled', false);
    $('#deleteFeatureMessage').text(
        "提示：确定要彻底删除功能：[" + featureName + "]及其全部内容么？该操作不可恢复");
    $('#deleteFeatureModal').modal('toggle');
}

// 执行删除功能（YBS）
function deleteFeature(featureId) {
    $("#ddbutton").attr('disabled', true);
    $('#deleteFeatureMessage').text("提示：正在删除，请稍候...");
    $.ajax({
        type: "POST",
        dataType: "text",
        data: {
        ...
        url: "homeController/deleteFeature.ajax",
        success: function (result) {
            if (result == "mustLogin") {
                window.location.href = "login.html";
            } else {
                if (result == "noAuthorized") {
                    $('#deleteFeatureMessage').text("提示：您的操作未被授权，删除文件夹失败");
                    $("#ddbutton").attr('disabled', false);
                } else if (result == "errorParameter") {
                    $('#deleteFeatureMessage').text("提示：参数不正确，删除文件夹失败");
                    $("#ddbutton").attr('disabled', false);
                } else if (result == "cannotDeleteFolder") {
                    $('#deleteFeatureMessage').text("提示：出现意外错误，可能未能删除文件夹");
                    $("#ddbutton").attr('disabled', false);
...
//显示修改功能模态框（YBS）
function showRenameFeatureModel(featureId, featureName) {
    $("#renameFeatureBox").html(
        "<button type='button' class='btn btn-primary' onclick='renameFeature("
        + '"' + featureId + '"' + ")'>修改</button>");
    $("#newfeaturename").val(featureName);
    $("#renameFeatureModal").modal('show');
}

// 执行修改功能（YBS）
function renameFeature(featureId) {
    var newFeatureName = $("#newfeaturename").val();
    var reg = new RegExp("[\/\|\\s\\\\\*\\<\\>\\?\\:\\&\\$" + '"' + "]+", "g");
    if (newFeatureName.length == 0) {
        showRFeatureAlert("提示：功能名称不能为空。");
    } else if (newFeatureName.length > 20) {
        showRFeatureAlert("提示：功能名称太长。");
    } else if (!reg.test(newFeatureName) && newFeatureName.indexOf(".") != 0) {
        $("#newfeaturealert").removeClass("alert");
        $("#newfeaturealert").removeClass("alert-danger");
        $("#featurerenamebox").removeClass("has-error");
        $("#newfeaturealert").text("");
        $.ajax({
            type: "POST",
            dataType: "text",
            data: {
            ...
            url: "homeController/renameFeature.ajax",
            success: function (result) {
                if (result == "mustLogin") {
                    window.location.href = "login.html";
                } else {
                    if (result == "noAuthorized") {
                        showRFeatureAlert("提示：您的操作未被授权，编辑失败。");
                    } else if (result == "errorParameter") {
                        showRFeatureAlert("提示：参数不正确，编辑失败，请刷新后重试。");
                    } else if (result == "nameOccupied") {
                        showRFeatureAlert("提示：该名称已被占用，请选取其他名称。");
                    } else if (result == "renameFeatureSuccess") {
                        $('#renameFeatureModal').modal('hide');
                        getFeature();
                        refresh();
                    } else {
                        showRFeatureAlert("提示：出现意外错误，可能未能编辑功能，请刷新后重试。");
                    }
                }
            },
            error: function () {
        ...
        showRFeatureAlert("提示：功能名中不应含有：空格 引号 / \ * | < > & $ : ? 且不能以“.”开头。");
    }
}
// 显示重命名功能状态提示（YBS）
function showRFeatureAlert(txt) {
    $("#newfeaturealert").addClass("alert");
    $("#newfeaturealert").addClass("alert-danger");
    $("#featurerenamebox").addClass("has-error");
    $("#newfeaturealert").text(txt);
}
...
// 显示重命名文件模态框
function showRenameFileModel(fileId, fileName, countryId, ids) {
    ...
    //初始化选中的国家
    $('#areaX.selectpicker').selectpicker('val', countryId);
    $('#areaX.selectpicker').selectpicker('refresh');

    //初始化选中的功能
    var oldnumber = new Array();
    oldnumber = ids.split(', ');
    $('#ffX.selectpicker').selectpicker('val', oldnumber);//默认选中
    $('#ffX.selectpicker').selectpicker('refresh');
