...
function showFolderTable(folderView) {
    ...
    $
        .each(
            folderView.fileAndFeatureViewList,
            function (n, fi) {
                fi.node.fileName = fi.node.fileName.replace('\'', '&#39;');
                var fileRow = "<tr id=" + fi.node.fileId + " onclick='checkfile(event," + '"'
                    + fi.node.fileId + '"' + ")' ondblclick='checkConsFile(event," + '"' + fi.node.fileId + '"' + ")' id='" + fi.node.fileId
                    + "' class='filerow'><td>" + fi.node.fileName
                    + "</td><td class='hiddenColumn'>" + fi.node.fileCreationDate + "</td>";
                if (fi.node.fileSize == "0") {
                    fileRow = fileRow + "<td>&lt;1MB</td>";
                } else {
                    fileRow = fileRow + "<td>" + fi.node.fileSize + "MB</td>";
                }
                fileRow = fileRow + "<td class='hiddenColumn'>";
                if (fi.country) {
                    fileRow = fileRow + fi.country.countryName;
                } else {
                    fileRow = fileRow + "--";
                }
                fileRow = fileRow + "</td>" + "<td>";

                var list = fi.featureList;
                var names = "";
                ...
                fileRow = fileRow + "</td>" + "<td class='hiddenColumn'>" + fi.node.fileCreator + "</td><td>";
                if (aL) {
                    fileRow = fileRow
                    ...
                    var suffix = getSuffix(fi.node.fileName);
                    switch (suffix) {
                        case "mp4":
                        case "webm":
                        case "mov":
